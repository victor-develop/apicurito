...
export class EditorComponent implements OnInit {
  ...
  public documentChanged(): any {
    console.info(
      "[EditorComponent] Detected a document change, scheduling disaster recovery persistence"
    );
  ...
  public convert(): void {
    ...
    setTimeout(() => {
      ...
      let doc20: Oas20Document = <Oas20Document>(
        Library.readDocument(currentApi.spec)
      );
  ...
  public generate(gconfig: GeneratorConfig): void {
    ...
    this.downloader
      .generateAndDownload(gconfig, content, filename)
      .then(() => {
